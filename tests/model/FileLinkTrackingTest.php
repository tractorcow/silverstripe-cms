...
class FileLinkTrackingTest extends SapphireTest {
	...
	public function testFileRenameUpdatesDraftAndPublishedPages() {
		...
		$this->assertContains('<img src="assets/testscript-test-file.pdf"',
			DB::preparedQuery("SELECT \"Content\" FROM \"SiteTree_Live\" WHERE \"ID\" = ?", array($page->ID))->value());
		...
		$this->assertContains('<img src="assets/renamed-test-file.pdf"',
			DB::preparedQuery("SELECT \"Content\" FROM \"SiteTree\" WHERE \"ID\" = ?", array($page->ID))->value());
		$this->assertContains('<img src="assets/renamed-test-file.pdf"',
			DB::preparedQuery("SELECT \"Content\" FROM \"SiteTree_Live\" WHERE \"ID\" = ?", array($page->ID))->value());
	}
	...
	public function testFileLinkRewritingOnVirtualPages() {
		...
		$this->assertContains('<img src="assets/renamed-test-file.pdf"',
			DB::preparedQuery("SELECT \"Content\" FROM \"SiteTree\" WHERE \"ID\" = ?", array($svp->ID))->value());
		$this->assertContains('<img src="assets/renamed-test-file.pdf"',
			DB::preparedQuery("SELECT \"Content\" FROM \"SiteTree_Live\" WHERE \"ID\" = ?", array($svp->ID))->value());
	}
